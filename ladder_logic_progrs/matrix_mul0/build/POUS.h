#include "beremiz.h"
#ifndef __POUS_H
#define __POUS_H

#include "accessor.h"
#include "iec_std_lib.h"

__DECLARE_ENUMERATED_TYPE(LOGLEVEL,
  LOGLEVEL__CRITICAL,
  LOGLEVEL__WARNING,
  LOGLEVEL__INFO,
  LOGLEVEL__DEBUG
)
__DECLARE_ARRAY_TYPE(__ARRAY_OF_DINT_4,DINT,[4])
// FUNCTION_BLOCK LOGGER
// Data part
typedef struct {
  // FB Interface - IN, OUT, IN_OUT variables
  __DECLARE_VAR(BOOL,EN)
  __DECLARE_VAR(BOOL,ENO)
  __DECLARE_VAR(BOOL,TRIG)
  __DECLARE_VAR(STRING,MSG)
  __DECLARE_VAR(LOGLEVEL,LEVEL)

  // FB private variables - TEMP, private and located variables
  __DECLARE_VAR(BOOL,TRIG0)

} LOGGER;

void LOGGER_init__(LOGGER *data__, BOOL retain);
// Code part
void LOGGER_body__(LOGGER *data__);
// FUNCTION_BLOCK ELEMENT_SELECTOR
// Data part
typedef struct {
  // FB Interface - IN, OUT, IN_OUT variables
  __DECLARE_VAR(BOOL,EN)
  __DECLARE_VAR(BOOL,ENO)
  __DECLARE_VAR(__ARRAY_OF_DINT_4,M)
  __DECLARE_VAR(UINT,INDEX)
  __DECLARE_VAR(DINT,OUT)

  // FB private variables - TEMP, private and located variables

} ELEMENT_SELECTOR;

void ELEMENT_SELECTOR_init__(ELEMENT_SELECTOR *data__, BOOL retain);
// Code part
void ELEMENT_SELECTOR_body__(ELEMENT_SELECTOR *data__);
// FUNCTION_BLOCK ELEMENT_SETTER
// Data part
typedef struct {
  // FB Interface - IN, OUT, IN_OUT variables
  __DECLARE_VAR(BOOL,EN)
  __DECLARE_VAR(BOOL,ENO)
  __DECLARE_VAR(__ARRAY_OF_DINT_4,M)
  __DECLARE_VAR(UINT,INDEX)
  __DECLARE_VAR(DINT,VALUE)
  __DECLARE_VAR(DINT,OUT)

  // FB private variables - TEMP, private and located variables

} ELEMENT_SETTER;

void ELEMENT_SETTER_init__(ELEMENT_SETTER *data__, BOOL retain);
// Code part
void ELEMENT_SETTER_body__(ELEMENT_SETTER *data__);
// PROGRAM MATRIX_MUL
// Data part
typedef struct {
  // PROGRAM Interface - IN, OUT, IN_OUT variables

  // PROGRAM private variables - TEMP, private and located variables
  __DECLARE_VAR(__ARRAY_OF_DINT_4,M_A)
  __DECLARE_VAR(__ARRAY_OF_DINT_4,M_B)
  __DECLARE_VAR(__ARRAY_OF_DINT_4,M_C)
  __DECLARE_VAR(UINT,I)
  __DECLARE_VAR(UINT,J)
  __DECLARE_VAR(UINT,K)
  __DECLARE_VAR(UINT,N)
  __DECLARE_VAR(UINT,INDEX_A)
  __DECLARE_VAR(UINT,INDEX_B)
  __DECLARE_VAR(UINT,INDEX_C)
  __DECLARE_VAR(DINT,ELEMENT_A)
  __DECLARE_VAR(DINT,ELEMENT_B)
  __DECLARE_VAR(DINT,ELEMENT_C)
  __DECLARE_VAR(DINT,CHECK)
  __DECLARE_VAR(BOOL,RUN_FLAG)
  ELEMENT_SELECTOR INPUT_MATRICES0;
  ELEMENT_SELECTOR INPUT_MATRICES1;
  ELEMENT_SETTER ELEMENT_SETTER0;
  ELEMENT_SELECTOR ELEMENT_SELECTOR0;
  ELEMENT_SELECTOR ELEMENT_SELECTOR1;
  ELEMENT_SELECTOR ELEMENT_SELECTOR2;
  ELEMENT_SELECTOR ELEMENT_SELECTOR3;
  __DECLARE_VAR(BOOL,_TMP_GE48_ENO)
  __DECLARE_VAR(BOOL,_TMP_GE48_OUT)
  __DECLARE_VAR(BOOL,_TMP_MUL43_ENO)
  __DECLARE_VAR(UINT,_TMP_MUL43_OUT)
  __DECLARE_VAR(BOOL,_TMP_ADD42_ENO)
  __DECLARE_VAR(UINT,_TMP_ADD42_OUT)
  __DECLARE_VAR(BOOL,_TMP_MUL6_ENO)
  __DECLARE_VAR(UINT,_TMP_MUL6_OUT)
  __DECLARE_VAR(BOOL,_TMP_ADD5_ENO)
  __DECLARE_VAR(UINT,_TMP_ADD5_OUT)
  __DECLARE_VAR(BOOL,_TMP_MUL19_ENO)
  __DECLARE_VAR(UINT,_TMP_MUL19_OUT)
  __DECLARE_VAR(BOOL,_TMP_ADD18_ENO)
  __DECLARE_VAR(UINT,_TMP_ADD18_OUT)
  __DECLARE_VAR(BOOL,_TMP_ADD46_ENO)
  __DECLARE_VAR(UINT,_TMP_ADD46_OUT)
  __DECLARE_VAR(BOOL,_TMP_MOVE49_ENO)
  __DECLARE_VAR(UINT,_TMP_MOVE49_OUT)
  __DECLARE_VAR(BOOL,_TMP_ADD30_ENO)
  __DECLARE_VAR(UINT,_TMP_ADD30_OUT)
  __DECLARE_VAR(BOOL,_TMP_GE34_ENO)
  __DECLARE_VAR(BOOL,_TMP_GE34_OUT)
  __DECLARE_VAR(BOOL,_TMP_MOVE40_ENO)
  __DECLARE_VAR(UINT,_TMP_MOVE40_OUT)
  __DECLARE_VAR(BOOL,_TMP_ADD58_ENO)
  __DECLARE_VAR(UINT,_TMP_ADD58_OUT)
  __DECLARE_VAR(BOOL,_TMP_GE52_ENO)
  __DECLARE_VAR(BOOL,_TMP_GE52_OUT)
  __DECLARE_VAR(BOOL,_TMP_MUL68_ENO)
  __DECLARE_VAR(DINT,_TMP_MUL68_OUT)
  __DECLARE_VAR(BOOL,_TMP_ADD69_ENO)
  __DECLARE_VAR(DINT,_TMP_ADD69_OUT)
  __DECLARE_VAR(BOOL,_TMP_MOVE71_ENO)
  __DECLARE_VAR(DINT,_TMP_MOVE71_OUT)

} MATRIX_MUL;

void MATRIX_MUL_init__(MATRIX_MUL *data__, BOOL retain);
// Code part
void MATRIX_MUL_body__(MATRIX_MUL *data__);
#endif //__POUS_H
